...
def yonghu_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是" and 'yonghu' != 'forum' :
            ...
            if tablename!="users" and tablename!="jdfnl" and 'yonghu'[:7]!='discuss' and "userid" in yonghu.getallcolumn(yonghu,yonghu):
                req_dict["userid"] = request.session.get("params").get("id")
...
def yonghu_update(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        if yonghu.objects.exclude(id=req_dict['id']).filter(zhanghao = req_dict['zhanghao']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "账号已存在"
