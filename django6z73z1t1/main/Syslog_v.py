...
def syslog_register(request):
    if request.method in ["POST", "GET"]:
        msg = {'code': normal_code, "msg": mes.normal_code}
        req_dict = request.session.get("req_dict")


        error = syslog.createbyreq(syslog, syslog, req_dict)
...
def syslog_login(request):
    if request.method in ["POST", "GET"]:
        msg = {'code': normal_code, "msg": mes.normal_code}
        req_dict = request.session.get("req_dict")
        datas = syslog.getbyparams(syslog, syslog, req_dict)
        if not datas:
        ...
        return Auth.authenticate(Auth, syslog, req_dict)


def syslog_logout(request):
    if request.method in ["POST", "GET"]:
        msg = {
            "msg": "登出成功",
            "code": 0
...
def syslog_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __loginUserColumn__= syslog.__loginUserColumn__
        except:
        ...
        records=syslog.getbyparams(syslog, syslog, recordsParam)
        if len(records)<1:
            msg['code'] = 400
            msg['msg'] = '用户不存在'
            return JsonResponse(msg, encoder=CustomJsonEncoder)

        eval('''syslog.objects.filter({}='{}').update({}='{}')'''.format(username_str,username,password_str,init_pwd))
        
        return JsonResponse(msg, encoder=CustomJsonEncoder)



def syslog_session(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        ...
        msg['data']  = syslog.getbyparams(syslog, syslog, req_dict)[0]
...
def syslog_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __hasMessage__=syslog.__hasMessage__
        except:
            __hasMessage__=None
        if  __hasMessage__=="是":
            tablename=request.session.get("tablename")
        ...
        # 当前表也是有管理员权限的表
        if  __isAdmin__ == "是" and 'syslog' != 'forum' :
            if req_dict.get("userid") and 'syslog' != 'chat' and 'syslog' != 'examrecord':
                del req_dict["userid"]
        else:
            if tablename!="users" and tablename!="jdfnl" and 'syslog'[:7]!='discuss' and "userid" in syslog.getallcolumn(syslog,syslog):
                req_dict["userid"] = request.session.get("params").get("id")
...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":{"currPage":1,"totalPage":1,"total":1,"pageSize":10,"list":[]}}
        req_dict = request.session.get("req_dict")
        if "clicknum"  in syslog.getallcolumn(syslog,syslog):
...
def syslog_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= syslog.createbyreq(syslog,syslog, req_dict)
...
def syslog_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="是"  and  "clicknum"  in syslog.getallcolumn(syslog,syslog):
            try:
...
def syslog_update(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if req_dict.get("mima") and "mima" not in syslog.getallcolumn(syslog,syslog) :
            del req_dict["mima"]
        if req_dict.get("password") and "password" not in syslog.getallcolumn(syslog,syslog) :
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    req_dict = {}
                    syslog.createbyreq(syslog, syslog, req_dict)
...
def syslog_security(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT * FROM syslog where ='{0}'".format(req_dict['username'])
        record = {}
