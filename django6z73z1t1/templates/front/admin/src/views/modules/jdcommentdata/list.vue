	...
	<div class="main-content" :style='{"width":"100%","padding":"20px 40px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","margin":"0 0px 20px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">评论ID</label>
						<el-input v-model="searchForm.commentid" placeholder="评论ID" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">商品ID</label>
						<el-input v-model="searchForm.productid" placeholder="商品ID" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">行业</label>
						<el-input v-model="searchForm.industry" placeholder="行业" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						...
						<el-input v-model="searchForm.brand" placeholder="品牌" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">商品系列</label>
						<el-input v-model="searchForm.productseries" placeholder="商品系列" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						...
						<el-input v-model="searchForm.model" placeholder="型号" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"0px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('jdcommentdata','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('jdcommentdata','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>

					<el-button class="btn18" v-if="isAuth('jdcommentdata','导入')" type="success" @click="importHandler()">
						<span class="icon iconfont icon-daoru11" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						导入
					</el-button>
					<el-button class="btn18" v-if="isAuth('jdcommentdata','上传模板')" type="success" @click="importClcik">
						<span class="icon iconfont icon-shangchuan8" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						上传模板
					</el-button>
					<el-button class="btn18" v-if="isAuth('jdcommentdata','下载模板')" type="success" @click="download('upload/jdcommentdata_template.xlsx')">
						<span class="icon iconfont icon-xiazai7" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						下载模板
					</el-button>

					<download-excel v-if="isAuth('jdcommentdata','导出')" class="export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "京东评论数据.xls">
						<!-- 导出excel -->
					...
					<el-button class="btn18" v-if="isAuth('jdcommentdata','品牌')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						品牌
					</el-button>
					<el-button class="btn18" v-if="isAuth('jdcommentdata','商品系列')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						商品系列
					</el-button>
					<el-button class="btn18" v-if="isAuth('jdcommentdata','型号')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						型号
					</el-button>
					<el-button class="btn18" v-if="isAuth('jdcommentdata','SKU属性')" type="success" @click="chartDialog4()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						SKU属性
					</el-button>
					<el-button class="btn18" v-if="isAuth('jdcommentdata','客户端')" type="success" @click="chartDialog5()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						客户端
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #fff","padding":"0","color":"#000","borderRadius":"10px","background":"#fff","borderWidth":"0px","width":"100%","fontSize":"14px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="merchantresponse"
						label="商家回复">
						<template slot-scope="scope">
							{{scope.row.merchantresponse}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="commentswithpictures"
						label="带图评论">
						<template slot-scope="scope">
							{{scope.row.commentswithpictures}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="followupreviewcontent"
						label="追评内容">
						<template slot-scope="scope">
							{{scope.row.followupreviewcontent}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="reviewtime"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jdcommentdata','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"12px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('jdcommentdata','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jdcommentdata','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog title="导入" :visible.sync="importVisiable" width="50%">
			<el-form ref="form" :model="form" label-width="80px">
				<el-form-item class="upload" label="文件" prop="excelFile">
				  <excel-file-upload
				  tip="点击上传直接导入excel文件"
				  action="jdcommentdata/importExcel"
				  :limit="1"
				  :fileUrls="importUrl"
				  @change="importChange"
				  ></excel-file-upload>
		...
		<el-dialog title="上传模板" :visible.sync="importVis" width="50%">
			<el-form ref="form" :model="importForm" label-width="80px">
				<el-form-item class="upload" label="文件" prop="excelFile">
					<el-upload class="upload-demo" drag :action="$base.url + 'file/upload?type=jdcommentdata_template'"
		...
		<el-dialog
			...
			width="800">
			<div id="brandChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="productseriesChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="modelChart3" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="skuattributeChart4" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="clientChart5" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
...
//导出excel
				json_fields: {
					...
					"行业": "industry",    //常规字段
					"品牌": "brand",    //常规字段
					"商品系列": "productseries",    //常规字段
					"型号": "model",    //常规字段
					"SKU属性": "skuattribute",    //常规字段
					"用户信息": "userinformation",    //常规字段
					"评价内容": "evaluationcontent",    //常规字段
					"评分明细": "ratingdetails",    //常规字段
					"客户端": "client",    //常规字段
					"评价时间": "evaluationtime",    //常规字段
					"商家回复": "merchantresponse",    //常规字段
					"带图评论": "commentswithpictures",    //常规字段
					"追评内容": "followupreviewcontent",    //常规字段
					"追评时间": "reviewtime",    //常规字段
				},
				json_meta: [
					[
						{
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var brandChart1 = echarts.init(document.getElementById("brandChart1"),'macarons');
					this.$http({
						url: "jdcommentdata/group/brand",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var productseriesChart2 = echarts.init(document.getElementById("productseriesChart2"),'macarons');
					this.$http({
						url: "jdcommentdata/group/productseries",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].productseries);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].productseries
								})
							}
							var option = {};
							...
							// 使用刚指定的配置项和数据显示图表。
							productseriesChart2.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								productseriesChart2.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog3() {
				...
				this.$nextTick(()=>{
					...
					var modelChart3 = echarts.init(document.getElementById("modelChart3"),'macarons');
					this.$http({
						url: "jdcommentdata/group/model",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
			...
			chartDialog4() {
				...
				this.$nextTick(()=>{
					...
					var skuattributeChart4 = echarts.init(document.getElementById("skuattributeChart4"),'macarons');
					this.$http({
						url: "jdcommentdata/group/skuattribute",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].skuattribute);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].skuattribute
								})
							}
							var option = {};
							let titleObj = this.bar.title
							...
							// 使用刚指定的配置项和数据显示图表。
							skuattributeChart4.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								skuattributeChart4.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog5() {
				...
				this.$nextTick(()=>{
					...
					var clientChart5 = echarts.init(document.getElementById("clientChart5"),'macarons');
					this.$http({
						url: "jdcommentdata/group/client",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.commentid!='' && this.searchForm.commentid!=undefined){
					params['commentid'] = '%' + this.searchForm.commentid + '%'
				}
 				if(this.searchForm.productid!='' && this.searchForm.productid!=undefined){
					params['productid'] = '%' + this.searchForm.productid + '%'
				}
 				if(this.searchForm.industry!='' && this.searchForm.industry!=undefined){
					params['industry'] = '%' + this.searchForm.industry + '%'
				}
 				if(this.searchForm.brand!='' && this.searchForm.brand!=undefined){
					params['brand'] = '%' + this.searchForm.brand + '%'
				}
 				if(this.searchForm.productseries!='' && this.searchForm.productseries!=undefined){
					params['productseries'] = '%' + this.searchForm.productseries + '%'
				}
